#include <stdio.h>
#include <stdarg.h>
#include <cstring>
#include <time.h>

#define debug(fmt, ...) fprintf(stderr, "line %d: " fmt "\n", __LINE__, __VA_ARGS__)

#define MAXLINES 5000   //max #lines to be  sorted
//char *lineptr[MAXLINES];    //pointers to text lines

//int readlines(char *lineptr[], int nlines);
int readlines(char *lineptr[], char buf[], int nlines);
void writelines(char *lineptr[], int nlines);

void qsort(char *lineptr[], int left, int right);

//sort input lines
int main(int argc, char **argv)
{
    int nlines; //number of input lines read

    char buf[50000];
    
    char *lineptr[5000];

    if ((nlines = readlines(lineptr, buf, MAXLINES)) >= 0)
    {
        qsort(lineptr, 0, nlines - 1);
        writelines(lineptr, nlines);
        return 0;
    }
    else
    {
        printf("error: input too big to sort\n");
        return 1;
    }
    return 0; 
}

#define ALLOCSIZE 10000     //size of available space

static char allocbuf[ALLOCSIZE];    //storage for alloc
static char *allocp = allocbuf;     //next free position

char *alloc(int n)      //retrun pointer to n characters
{
    if (allocbuf + ALLOCSIZE - allocp >= n)     //if fits
    {
        allocp += n;
        return allocp - n;  //old p
    }
    else
    {
        return 0;   //not  enough room
    }
}

void afree(char *p)
{
    if (p >= allocbuf && p < allocbuf + ALLOCSIZE)
        allocp = p;
    
    return ;
}

#define MAXLEN 1000 //max length of any input line
int getline(char *, int);
char *alloc(int);

//readlines:    read input lines
/* int readlines(char *lineptr[], int maxlines)
{
    int len, nlines;
    char *p, line[MAXLEN];

    nlines = 0;
    while ((len = getline(line, MAXLEN)) > 0)
    {
        if  (nlines >= maxlines || (p = alloc(len)) == NULL)
            return -1;
        else
        {
            line[len - 1] = '\0';   //delete newline
            strcpy(p, line);
            lineptr[nlines++] = p;
        }
    }
    return nlines;
} */

int readlines(char *lineptr[], char buf[], int maxlines)
{
    int len, nlines;
    char *bufp = buf;   //next free position
    char line[MAXLEN];

    nlines = 0;
    while ((len = getline(line, MAXLEN)) > 0)
    {
        if  (nlines >= maxlines)
            return -1;
        else
        {
            line[len - 1] = '\0';   //delete newline
            strcpy(bufp, line);
            //debug("%s",  bufp + p);
            lineptr[nlines++] = bufp;
            bufp += len;
        }
    }
    return nlines;
}

//readlines the second version

//getline : read a line into s, return length
int getline(char s[], int lim)
{
    int c, i;

    for (i = 0; i < lim - 1 && (c = getchar()) != EOF && c != '\n'; i++)
        s[i] = c;
    if (c == '\n')  {
        s[i] = c;
        ++i;
    }
    s[i] = '\0';
    return i;
}

void writelines(char *lineptr[], int nlines)
{
    int i;
    for (i = 0; i < nlines; i++)
        printf("%s\n", lineptr[i]);

    return ;
}

//qsort:    sort v[left]  ... v[right] into increasing order
void qsort(char *v[], int left, int right)
{
    int i, last;
    void swap(char *v[], int i, int j);

    if (left >= right)  //do nothing if array contains
        return;         //fewer than two elenments
    swap(v, left, (left + right) / 2);  //选择第一个元素
    last = left;
    for (i = left + 1; i <= right; i++)
    {
        if (strcmp(v[i], v[left]) < 0)
            swap(v, ++last, i);
    }
    swap(v, left, last);    //交换结束后，last位置左边的元素是小于右边的元素的
    qsort(v, left, last - 1);
    qsort(v, last + 1, right);
    return;
}

//swap: interchange v[i] and v[j]
void swap(char *v[], int i, int j)
{
    char *temp;

    temp = v[i];
    v[i] = v[j];
    v[j] = temp;
    return; 
}
